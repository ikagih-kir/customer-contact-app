...
#【問題1】の追加箇所3つ
#製品マニュアル
def run_product_manual_chain(param):
    ai_msg = st.session_state.product_manual_chain.invoke(
...
#営業FAQ
def run_sales_faq_chain(param):
    ai_msg = st.session_state.sales_faq_chain.invoke(
        {"input": param, "chat_history": st.session_state.chat_history}
    )
    st.session_state.chat_history.extend([
        HumanMessage(content=param),
...
#クレーム / トラブル対応
def run_trouble_support_chain(param):
    ai_msg = st.session_state.trouble_support_chain.invoke(
        {"input": param, "chat_history": st.session_state.chat_history}
    )
    st.session_state.chat_history.extend([
        HumanMessage(content=param),
...
def notice_slack(chat_message):
    ...
    reason_prompt = ChatPromptTemplate.from_messages([
            ...
            "system",
            """
            あなたは問い合わせ担当者を選定するAIアシスタントです。
            以下の従業員情報をもとに、なぜ {slack_id_text} の担当者がこの問い合わせに最適なのか、
            簡潔かつ論理的に説明してください。

            【生成条件】
            - 3〜5文で、具体的に述べること。
            - 「過去の問い合わせ対応履歴」「対応可能な問い合わせカテゴリ」「主要業務」を重視して説明する。
            - 自信を持って明確に述べる。
            """
            ),
            (
    ...
    reason_messages = reason_prompt.format_prompt(
        slack_id_text=slack_id_text, query=chat_message, context=context
    ).to_messages()

    # LLMで理由生成
    selection_reason = st.session_state.llm.invoke(reason_messages).content
    ...
    prompt = PromptTemplate(
        input_variables=["slack_id_text", "query", "context", "now_datetime", "selection_reason"],
