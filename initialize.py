...
def initialize_agent_executor():
    ...
    st.session_state.llm = ChatOpenAI(model_name=ct.MODEL, temperature=ct.TEMPERATURE, streaming=False)
    ...
    #【問題1】の追加箇所3つ
    #製品マニュアル（serviceフォルダ）
    st.session_state.product_manual_chain = utils.create_rag_chain(ct.DB_SERVICE_PATH)
    #営業FAQ（serviceフォルダ：営業情報は通常サービス資料）
    st.session_state.sales_faq_chain = utils.create_rag_chain(ct.DB_SERVICE_PATH)
    #クレーム/トラブル対応（customerフォルダ）
    st.session_state.trouble_support_chain = utils.create_rag_chain(ct.DB_CUSTOMER_PATH)
    ...
    tools = [
        ...
        # 製品マニュアル Tool
        Tool(
            name=ct.SEARCH_PRODUCT_MANUAL_TOOL_NAME,
            func=utils.run_product_manual_chain,
            description=ct.SEARCH_PRODUCT_MANUAL_TOOL_DESCRIPTION
        ),
        # 営業FAQ Tool
        Tool(
            name=ct.SEARCH_SALES_FAQ_TOOL_NAME,
            func=utils.run_sales_faq_chain,
            description=ct.SEARCH_SALES_FAQ_TOOL_DESCRIPTION
        ),
        # クレーム/トラブル対応 Tool
        Tool(
            name=ct.SEARCH_TROUBLE_SUPPORT_TOOL_NAME,
            func=utils.run_trouble_support_chain,
            description=ct.SEARCH_TROUBLE_SUPPORT_TOOL_DESCRIPTION
        )
    ]
